...
latPoint = [eyePoint[0]+Math.cos(theta),eyePoint[1]+Math.sin(theta),1]
...
function updatePointInput() {
  inputs = ['px','py','pz','ar','ag','ab','dr','dg','db','sr','sg','sb']
...
function drawResize() {
  g_canvasID.width = innerWidth - 32
  g_canvasID.height = 4*innerHeight/5 - 32
...
var b4Wait = b4Draw - g_lastMS;
    ...
    vpAspect = g_canvasID.width/g_canvasID.height
    latPoint[0] = eyePoint[0]+Math.cos(theta)
    latPoint[1] = eyePoint[1]+Math.sin(theta)

    worldBox.switchToMe();
    worldBox.draw();
    if(g_show1 == 1) { // IF user didn't press HTML button to 'hide' VBO1:
    part1Box.switchToMe();  // Set WebGL to render from this VBObox.
    part1Box.draw(WorldMat);			  // draw our VBO's contents using our shaders.
    }
...
// Called when user presses HTML-5 button 'Show/Hide VBO1'.
  if(g_show1 != 1) { g_show1 = 1;	console.log('phong'); }
  else { g_show1 = 0; console.log('gouraud'); }
}

function listenKeyDown(kev) {
  // console.log(kev.code)
  switch (kev.code) {
      case "KeyR":
          animAngle = 0
          console.log('reset')
          break
      case "KeyW":
          // move in camera dir by set distance
          var dist = 0.2
          var offset = [dist * (latPoint[0] - eyePoint[0]),
                          dist * (latPoint[1] - eyePoint[1]),
                          dist * (latPoint[2] - eyePoint[2])]
          eyePoint[0] += offset[0]
          eyePoint[1] += offset[1]
          eyePoint[2] += offset[2]
          latPoint[0] += offset[0]
          ...
          break
      case "KeyS":
          // move opposite camera dir by set distance
          var dist = 0.2
          var offset = [dist * (latPoint[0] - eyePoint[0]),
                          dist * (latPoint[1] - eyePoint[1]),
                          dist * (latPoint[2] - eyePoint[2])]
      ...
      case "KeyA":
          // strafe left
          var dist = 0.2
          var offset = [dist * Math.cos(theta+Math.PI/2),
                          dist * Math.sin(theta+Math.PI/2)]
          eyePoint[0] += offset[0]
          eyePoint[1] += offset[1]
          latPoint[0] += offset[0]
          latPoint[1] += offset[1]
