...
// easily WITHOUT disrupting any other code, ever!
  ...
  groundVerts = new Float32Array(floatsPerVertex*2*(xcount+ycount));
  ...
  for(v=0, j=0; v<2*xcount; v++, j+= floatsPerVertex) {
      if(v%2==0) {	// put even-numbered vertices at (xnow, -xymax, 0)
          groundVerts[j  ] = -xymax + (v  )*xgap;	// x
          groundVerts[j+1] = -xymax;								// y
          groundVerts[j+2] = 0.0;									// z
          groundVerts[j+3] = 1.0;									// w.
      }
      else {				// put odd-numbered vertices at (xnow, +xymax, 0).
          groundVerts[j  ] = -xymax + (v-1)*xgap;	// x
          groundVerts[j+1] = xymax;								// y
          groundVerts[j+2] = 0.0;									// z
          groundVerts[j+3] = 1.0;									// w.
      }
      groundVerts[j+4] = xColr[0];			// red
      groundVerts[j+5] = xColr[1];			// grn
      groundVerts[j+6] = xColr[2];			// blu
  }
  // Second, step thru y values as wqe make horizontal lines of constant-y:
  // (don't re-initialize j--we're adding more vertices to the array)
  for(v=0; v<2*ycount; v++, j+= floatsPerVertex) {
      if(v%2==0) {		// put even-numbered vertices at (-xymax, ynow, 0)
          groundVerts[j  ] = -xymax;								// x
          groundVerts[j+1] = -xymax + (v  )*ygap;	// y
          groundVerts[j+2] = 0.0;									// z
          groundVerts[j+3] = 1.0;									// w.
      }
      else {					// put odd-numbered vertices at (+xymax, ynow, 0).
          groundVerts[j  ] = xymax;								// x
          groundVerts[j+1] = -xymax + (v-1)*ygap;	// y
          groundVerts[j+2] = 0.0;									// z
          groundVerts[j+3] = 1.0;									// w.
      }
      groundVerts[j+4] = yColr[0];			// red
      groundVerts[j+5] = yColr[1];			// grn
      groundVerts[j+6] = yColr[2];			// blu
  }

  this.vboContents = groundVerts

	this.vboVerts = groundVerts.length/7;						// # of vertices held in 'vboContents' array
...
VBObox0.prototype.adjust = function(WorldMat) {
//==============================================================================
...
VBObox0.prototype.draw = function(WorldMat) {
//=============================================================================
  ...
  // this.ModelMat.setTranslate(0.35, 0, 0);							// then translate them.
  pushMatrix(this.ModelMatrix)
//   this.ModelMatrix.scale(0.1,0.1,0.1)
  this.ModelMatrix.perspective(vfov,vpAspect,znear,zfar)
  this.ModelMatrix.lookAt(  eyePoint[0], eyePoint[1], eyePoint[2],
                            latPoint[0], latPoint[1], latPoint[2],
...
//  '	 vec3 emissive = u_Ke;' +
 '	 vec3 emissive = 										u_MatlSet[0].emit;' +
  ...
  this.matl1 = new Material(MATL_PEARL)
  this.matl2 = new Material(MATL_BLU_PLASTIC)
  this.matl3 = new Material(MATL_SILVER_SHINY)
};
...
VBObox1.prototype.init = function() {
  ...
  this.n_vcount = this.initVertexBuffer(gl)
  if (this.n_vcount < 0) {
  ...
  this.lamp1.u_pos  = gl.getUniformLocation(gl.program, 'u_LampSet[1].pos');	
  this.lamp1.u_ambi = gl.getUniformLocation(gl.program, 'u_LampSet[1].ambi');
  this.lamp1.u_diff = gl.getUniformLocation(gl.program, 'u_LampSet[1].diff');
  this.lamp1.u_spec = gl.getUniformLocation(gl.program, 'u_LampSet[1].spec');
  ...
  this.eyePosWorld.set([eyePoint[0], eyePoint[1], eyePoint[2]]);
	gl.uniform3fv(this.uLoc_eyePosWorld, this.eyePosWorld);// use it to set our uniform

  this.lamp1.I_pos.elements.set(eyePoint);
...
VBObox1.prototype.initVertexBuffer = function(gl) {
  ...
  if (!this.initArrayBuffer(gl, 'a_Position', new Float32Array(this.positions), gl.FLOAT, 3)) return -1;
  if (!this.initArrayBuffer(gl, 'a_Normal', new Float32Array(this.positions), gl.FLOAT, 3))  return -1;
...
VBObox1.prototype.draw = function(WorldMat) {
//=============================================================================
// Send commands to GPU to select and render current VBObox contents.
  ...
  gl.uniform3fv(this.lamp0.u_pos,  pointPos);
  //		 ('slice(0,3) member func returns elements 0,1,2 (x,y,z) ) 
  gl.uniform3fv(this.lamp0.u_ambi, pointAmbi);		// ambient
  gl.uniform3fv(this.lamp0.u_diff, pointDiff);		// diffuse
  gl.uniform3fv(this.lamp0.u_spec, pointSpec);		// Specular
  //	console.log('lamp0.u_pos',lamp0.u_pos,'\n' );
  //	console.log('lamp0.I_diff.elements', lamp0.I_diff.elements, '\n');

  gl.uniform3fv(this.lamp1.u_pos,  eyePoint);
  //		 ('slice(0,3) member func returns elements 0,1,2 (x,y,z) ) 
  ...
  this.mvpMatrix.setPerspective(vfov, vpAspect, znear, zfar);
  this.mvpMatrix.lookAt(eyePoint[0], eyePoint[1], eyePoint[2], 
                        latPoint[0], latPoint[1], latPoint[2],
                        upVector[0], upVector[1], upVector[2]);
  ...
  gl.drawElements(gl.TRIANGLES, this.n_vcount, gl.UNSIGNED_SHORT, 0);
  ...
  this.modelMatrix.setTranslate(0,-3,0.3)
  this.modelMatrix.scale(0.3,0.3,0.3)
  this.modelMatrix.rotate(g_angleNow1-105,1,0,0)
  ...
  this.modelMatrix.translate(0,0,1.9)
  this.modelMatrix.rotate(g_angleNow1-105,1,0,0)
  ...
  this.modelMatrix.setTranslate(0,-6,1)
  ...
  this.modelMatrix.rotate(g_angleNow1-15,1,0,0)
  this.modelMatrix.translate(0,0,1.6)
  ...
  this.modelMatrix = popMatrix()
  this.modelMatrix.rotate(g_angleNow1-195,1,0,0)
  this.modelMatrix.translate(0,0,1.6)
  this.modelMatrix.scale(0.8,0.8,0.8)
  ...
  this.modelMatrix.setTranslate(0,-9,1)
  ...
  this.modelMatrix.rotate(g_angleNow2,0,1,0)
  this.modelMatrix.rotate(g_angleNow1-105,1,0,0)
  this.modelMatrix.translate(0,0,1.2)
  this.modelMatrix.scale(0.3,0.3,0.3)
...
var VSHADER_SOURCE =
    ...
    '  vec3 headLightDirection = normalize(u_LampSet[1].pos - vec3(vertexPosition));\n' +
    ...
    '  vec3 ambient = u_LampSet[0].ambi * u_MatlSet[0].ambi;\n' +
    '  vec3 speculr = u_LampSet[0].spec * u_MatlSet[0].spec * e64;\n' +
    '  vec3 diffuse2 = u_LampSet[1].diff * u_MatlSet[0].diff * nDotHL;\n' +
    '  vec3 speculr2 = u_LampSet[1].spec * u_MatlSet[0].spec * e64_2;\n' +
    // Add the surface colors due to diffuse reflection and ambient reflection
    '  v_Color = vec4(diffuse + ambient + speculr + diffuse2 + speculr2, 1);\n' + 
    '}\n';

    this.VERT_SRC =	VSHADER_SOURCE
...
//  you must call this VBObox object's switchToMe() function too!
  ...
  //  else{console.log('You called: '+ this.constructor.name + '.init() fcn!');}
    ...
    this.n = this.initVertexBuffer(gl)
    ...
    if (!this.uLoc_eyePosWorld ||
      !this.u_ModelMatrix	|| !this.u_MvpMatrix || !this.u_NormalMatrix) {
    console.log('Failed to get GPUs matrix storage locations');
    return;
    }
...
VBObox2.prototype.initVertexBuffer = function(gl) {
    var SPHERE_DIV = 13;
...
VBObox2.prototype.initArrayBuffer = function(gl, attribute, data, type, num) {
    // Create a buffer object
    this.buffer = gl.createBuffer();
    if (!this.buffer) {
        console.log('Failed to create the buffer object');
