...
The solution we came to quickly was a simple ring around the play field, which the player could click and drag to rotate the view. Early testing immediately showed that users did not pick up on the use of the ring and wrote it off as part of the environment. We mitigated this by adding a sphere-shaped 'handle' to act as a focal point, and further improved its noticeability by adding feedback, increasing its opacity when hovered.  
